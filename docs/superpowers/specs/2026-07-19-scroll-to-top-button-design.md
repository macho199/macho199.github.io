...
- PC·태블릿·모바일 콘텐츠의 오른쪽 선에서 12px 안쪽으로 들여쓴 고정 위치
- iPhone 안전 영역
...
- 화살표는 22×22px이며 현재 블로그의 연초록 accent를 사용한다.
- 중앙 콘텐츠 영역의 우측 하단에 `position: fixed`로 둔다.
- 버튼의 오른쪽 끝을 콘텐츠의 오른쪽 선에서 12px 안쪽으로 들여 두 경계에 붙어 보이는 느낌을 줄인다.
- 기존 헤더보다 위에 표시되는 명시적 z-index를 사용한다.

### 위치

- 1021px 이상 PC에서는 920px 컨테이너의 좌·우 48px 여백을 제외한 824px 콘텐츠 오른쪽 선에서 버튼을 12px 안쪽으로 이동한다.
- 721~1020px에서는 현재 24px 태블릿 컨테이너 여백에 12px를 더해 우측 간격을 36px로 둔다.
- 720px 이하에서는 24px 폰 컨테이너 여백과 12px 안쪽 여백, `env(safe-area-inset-right, 0px)`를 더해 정렬한다.
- 하단 간격은 721px 이상에서 24px에 12px를 더한 36px, 720px 이하에서 16px에 12px와 `env(safe-area-inset-bottom, 0px)`를 더한 28px+safe area다.
- safe-area inset이 0px인 경우 우측·하단 좌표는 390px viewport 36px·28px, 900px viewport 36px·36px, 1440px viewport 320px·36px이다.
- post 목차는 본문 오른쪽 밖에서 시작하므로 1440px에서 버튼과 60px 간격을 유지한다.
...
- CSS 48px 크기·22px 아이콘·콘텐츠 우측 12px inset·고정 위치·safe area·숨김·노출·hover·active 상태를 확인한다.
...
- PC·태블릿·모바일에서 버튼이 콘텐츠 오른쪽 선에서 12px 안쪽에 놓이는지 확인한다.
- 390px·900px·1440px viewport에서 각 36px·28px, 36px·36px, 320px·36px 우측·하단 좌표를 확인한다.
- post 목차와 버튼이 겹치지 않고 1440px에서 60px 간격을 유지하는지 확인한다.
- 모바일 safe area와 페이지 가로 overflow 0을 확인한다.

## 완료 조건

- index와 post에서 승인된 방향·거리 규칙으로 버튼이 노출·숨김된다.
- 버튼은 중앙 콘텐츠 오른쪽 선에서 12px 안쪽, 기존 하단에서 12px 위에 놓인 48×48px 원형 UI로 동작한다.
