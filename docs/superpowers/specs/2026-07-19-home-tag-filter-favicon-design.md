...
- 배포본 `static/favicon.png`의 바깥 테두리와 연결된 흰색 영역만 투명하게 바꾼다.
- 분홍색 원, 블록 그림, 원 안쪽의 흰색 명암은 변경하지 않는다.
- 캔버스 크기와 PNG 형식은 128×128로 유지하고 RGBA 알파 채널을 사용한다.
- 원본 보관 파일은 수정하지 않고 배포본만 교체한다.
- 검증은 PNG 알파 채널 존재, 네 모서리의 완전 투명, 중앙 픽셀의 불투명, 브라우저 탭의 비정사각형 표현을 확인한다.
...
- favicon 바깥 모서리는 투명하고 분홍색 원과 블록 그림은 원본 모양을 유지한다.
