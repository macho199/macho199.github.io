...
- 화면 우측 하단에 `position: fixed`로 둔다.
- 기존 헤더보다 위에 표시되는 명시적 z-index를 사용한다.

### 위치

- 721px 이상에서는 오른쪽 24px, 아래 24px이다.
- 720px 이하에서는 기본 오른쪽 16px, 아래 16px이다.
- 모바일 실제 위치에는 `env(safe-area-inset-right, 0px)`와 `env(safe-area-inset-bottom, 0px)`를 각각 더한다.
...
- CSS 크기·고정 위치·safe area·숨김·노출·hover·active 상태를 확인한다.
...
- 버튼은 화면 우측 하단에서 44×44px 원형 UI로 동작한다.
